...
# 1. On line number 14, we kindly request you to open the text file that contains the output log data generated by the ASTOR program.
# 2. Graphs are generated automatically as part of the process.
# 3. The data from each iteration is saved in JSON format in generation_mapping file created in the same folder, which was used for our comprehensive analysis. You can reviewing this data for reference.
# 4. Also check the console logs for more details. Important information is highlighted in red

import matplotlib.pyplot as plt
import numpy as np


# Open the text file containing the output log from astor
with open("outputLog.txt", "r") as file:
# with open("dfsOutput", "r") as file:
    ...
    def checkInitialVariants(initialProgramVariant):
        ...
        if all_equal:
            printHighlight("All variants of initial population are equal.")
    ...
    def suspiciousLinesGraph(suspicious_lines,suspiciousLinesSize):
        ...
        plt.xticks(line_numbers,rotation=90,fontsize=11)
    ...
    # Extract the variant IDs and fitness values
        ...
        if fitness_values:
            plt.yticks(range(max(min(fitness_values),0), max(fitness_values) + 1),fontsize=11)
    ...
    def checkDuplicatesInOperations(generationWiseMapping):
        ...
        if duplicates:
            ...
            # printHighlight("duplicate operations ids:"+str(duplicateIds))
    ...
    def checkDuplicatesInSolutions(solutionSummary):
        ...
        if duplicates:
            ...
            printHighlight("No duplicate solution found.")
    ...
    def lineModifications(generationWiseMapping):
        countLineObj={}
        for generation in generationWiseMapping.values():
            for variantId,variant_data in generation.items():
                if "lineNo" in variant_data:
                    if  "App.java"+variant_data["lineNo"] in countLineObj:
                        countLineObj["App.java"+variant_data["lineNo"]]+=1
                    else:
                        countLineObj["App.java"+variant_data["lineNo"]]=1
        newX=[]
        newY=[]
        for lineNo, number in countLineObj.items():
            newX.append(lineNo)
            newY.append(number)
        plt.figure(figsize=(10, 9))  # Adjust the figure size as needed
        plt.bar(newX, newY,color='red',width=0.5)
        plt.xlabel('Location'),
        plt.ylabel('Frequency of Patches Applied')
        plt.xticks(rotation=20,fontsize=11)
        # Adding labels on top of each bar
        for i, v in enumerate(newY):
            plt.text(newX[i], v, str(v), ha='center', va='bottom')
        # Show the plot
        plt.show()


        # printHighlight(str(countLineObj))
    ...
    lineModifications(generationWiseMapping)
    fitnessGraph(generationWiseMapping)
    printHighlight("solutionSummary")
    print(solutionSummary)
